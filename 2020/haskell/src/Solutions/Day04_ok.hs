...
module Solutions.Day04_ok where
...
parseInput = map mkPassport . splitOn "\n\n"
  where
    mkPassport = M.fromList . parseWith entries
...
part1 passports =
  countTrue
    [ all (`M.member` passport) requiredKeys
      | passport <- passports
    ]
...
part2 = countTrue . map isValid
  where
    isValid :: M.HashMap String String -> Bool
    isValid passport = null $
      execValidate $ do
...
validateHcl = \case
  ('#' : s) -> do
    unless (all (`elem` ("0123456789abcdef" :: String)) s) $
